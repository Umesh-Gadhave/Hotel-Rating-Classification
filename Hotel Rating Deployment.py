...
def Load_Model():
    return keras.models.load_model(f"{os.path.realpath(os.path.join(os.path.dirname(__file__),'BERT.h5'))}", custom_objects={'KerasLayer': hub.KerasLayer})
model = Load_Model()
...
def image(pred):
    if pred < 0.3:
        ...
        return st.image(
            '2.png', width=100, caption='Positive')
